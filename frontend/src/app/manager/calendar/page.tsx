...
export default function CalendarView() {
    ...
    const deleteCalendarEvent = async (taskId: string) => {
        try {
            ...
            const { data: calendarEvent } = await axios.get(`/api/calendar-events?task_id=${taskId}`);
            ...
            if (calendarEvent && calendarEvent.length > 0) {
                ...
                try {
                    await gapi.client.calendar.events.delete({
                        calendarId: 'primary',
                        eventId: calendarEvent[0].external_id,
                    });
                } catch (error) {
                    console.error('Error deleting from Google Calendar:', error);
                }

                // Delete from database using backend endpoint
                await axios.delete(`/api/calendar-events/${calendarEvent[0].external_id}`);
            ...
            for (const assignment of assignments) {
                ...
                try {
                    ...
                    if (!existingEventMap.has(task.id)) {
                        ...
                        const { error: insertError } = await axios.post('/api/calendar/events', {
                            task_id: task.id,
    ...
    const fetchCalendarEvents = async () => {
        try {
            const { data: calendarEvents } = await axios.get('/api/calendar/events');
