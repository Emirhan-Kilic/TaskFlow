        ...
        const fetchUpcomingDeadlines = async () => {
            try {
                ...
                const transformedTasks = tasks
                    .filter(task => task.task_assignments.some(
                        assignment => assignment.assigned_to.id === userData.id
                    ))
                    .map((task: TaskResponse) => ({
                        ...task,
                        task_assignments: task.task_assignments
                            .filter(assignment => assignment.assigned_to.id === userData.id),
                ...
                <header className="bg-slate-800/50 backdrop-blur-xl border-b border-slate-700/50">
                    <div className="container mx-auto px-4 py-4">
                        <div className="flex items-center justify-between mb-4">
                            <h1 className="text-2xl font-light text-white">My Upcoming Deadlines</h1>
                            <button
