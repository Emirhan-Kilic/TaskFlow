...
const DependencyGraph = ({ tasks, dependencies }: {
    ...
    // Update critical path highlighting to use backend API
    const highlightCriticalPath = async () => {
        try {
            const response = await fetch('/api/tasks/critical-path', {
                method: 'POST',
                headers: {
...
const BulkActions: React.FC<BulkActionsProps> = ({
    ...
    // Update status change handler to use backend API
                        ...
                        return (
                            <div key={task.id} className="relative">
                                ...
                                <div className="flex items-start gap-4">
                                    ...
                                    {/* Task content */}
                                    <div className="flex-1 bg-slate-700/50 rounded-xl p-4 ml-4 hover:bg-slate-700/70 transition-colors">
                                        <div className="flex justify-between items-start mb-2">
                                            <h4 className="text-white font-medium">{task.title}</h4>
                                        ...
                                        {task.task_assignments[0]?.comments && (
                                            <div className="mt-2 text-sm text-slate-400">
                                                <p className="line-clamp-2">{task.task_assignments[0].comments}</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
...
export default function TaskKanbanBoard() {
    ...
    const handleEditTask = async (taskId: bigint, data: Task) => {
        try {
            setIsLoading(true);
            ...
            // Update task assignment comments
            if (data.task_assignments[0]) {
                const { error: assignmentError } = await supabase
                    .from('task_assignments')
                    .update({
                        comments: data.task_assignments[0].comments,
                        updated_at: new Date().toISOString()
                    })
                    .eq('id', data.task_assignments[0].id);
            ...
            const { data: updatedTasks, error: refreshError } = await supabase
                ...
                .eq('task_assignments.assigned_to', userData?.id);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Update query to only fetch tasks assigned to the current user
                ...
                // Fetch dependencies only for the user's tasks
                ...
                // Only fetch team members from the same department
                    ...
                    <header className="bg-slate-800/50 backdrop-blur-xl border-b border-slate-700/50">
                        <div className="container mx-auto px-4 py-4">
                            <div className="flex items-center justify-between">
                                <h1 className="text-2xl font-light text-white">My Tasks</h1>
                                <button
